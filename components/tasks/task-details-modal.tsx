    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="w-full max-w-4xl p-0 gap-0 overflow-hidden flex flex-col max-h-[calc(100vh-2rem)]">
                {/* Header */}
                <div className="p-4 md:p-6 border-b flex flex-col md:flex-row md:items-start md:justify-between gap-3 bg-background shrink-0">
                ...
                {/* Unified Tabs System - Works on both Mobile & Desktop */}
                <Tabs defaultValue="comments" className="flex flex-col flex-1 overflow-hidden">
                    <TabsList className="w-full overflow-x-auto flex-nowrap rounded-none border-b bg-background shrink-0 p-0 h-auto">
                        <TabsTrigger value="comments" className="text-xs md:text-sm px-3 md:px-4 py-2.5 data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none">
                            Commentaires
                        </TabsTrigger>
                        <TabsTrigger value="description" className="text-xs md:text-sm px-3 md:px-4 py-2.5 data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none">
                            Description
                        </TabsTrigger>
                        <TabsTrigger value="subtasks" className="text-xs md:text-sm px-3 md:px-4 py-2.5 data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none">
                            Sous-Tâches
                        </TabsTrigger>
                        <TabsTrigger value="files" className="text-xs md:text-sm px-3 md:px-4 py-2.5 data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none">
                            Fichiers
                        </TabsTrigger>
                        <TabsTrigger value="details" className="text-xs md:text-sm px-3 md:px-4 py-2.5 data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none">
                            Détail
                        </TabsTrigger>
                    </TabsList>

                    {/* Comments Tab */}
                    <TabsContent value="comments" className="flex-1 overflow-y-auto p-0 m-0 data-[state=active]:flex data-[state=active]:flex-col">
                                    ...
                                    comments.map(comment => (
                                        <div key={comment.id} className="flex gap-3 text-sm">
                                            ...
                                            <div className="flex-1 space-y-1 min-w-0">
                                                ...
                                                {comment.content && (
                                                    <p className="text-muted-foreground leading-snug text-sm whitespace-pre-wrap">{comment.content}</p>
                                                )}
                    ...
                    {/* Description Tab */}
                    <TabsContent value="description" className="flex-1 overflow-y-auto p-4 md:p-6 m-0">
                    ...
                    {/* Subtasks Tab */}
                    <TabsContent value="subtasks" className="flex-1 overflow-y-auto p-4 md:p-6 m-0">
                        <div className="space-y-2">
                            {task.subtasks?.map((st: any) => (
                                <div key={st.id} className="flex items-start gap-3 p-2 rounded-md hover:bg-muted/50 group">
                                    <div
                                        className={`mt-0.5 cursor-pointer h-5 w-5 border rounded flex items-center justify-center shrink-0 ${st.completed ? 'bg-primary border-primary text-primary-foreground' : 'border-muted-foreground'}`}
                    ...
                    {/* Files Tab */}
                    <TabsContent value="files" className="flex-1 overflow-y-auto p-4 md:p-6 m-0">
                    ...
                    {/* Details Tab */}
                    <TabsContent value="details" className="flex-1 overflow-y-auto p-4 md:p-6 m-0 space-y-6">
                        {/* Finances */}
                        ...
                        <section className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="p-3 bg-muted/20 rounded-lg">
                                ...
                                <div className="flex -space-x-2">
                                    ...
                                    {(!task.assignees || task.assignees.length === 0) && <span className="text-sm">-</span>}
                                </div>
                            </div>
                            <div className="p-3 bg-muted/20 rounded-lg">
